...
function RichText({ text }: { text: string }) {
  ...
  while ((match = re.exec(text)) !== null) {
    ...
    if (href.startsWith("/")) {
      ...
      parts.push(
        <a key={key++} href={href} className="text-[var(--accent)] underline-offset-2 hover:underline" rel="noopener noreferrer">
          {label}
        </a>,
      );
    }
    ...
    <>
      ...
      <Section>
        <article className="prose-content mx-auto max-w-3xl">
          <div className="mb-8 overflow-hidden rounded-lg border border-[var(--line)]">
            <Image
              src={post.image}
              alt={post.imageAlt}
              width={1600}
          ...
          {post.blocks.map((block, index) => {
            ...
            if (block.type === "disclaimer") {
              return (
                <p key={index} className="mt-10 text-sm italic text-[var(--muted)]">
